...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{ route('statements.update',['statement'=>$statement]) }}" autocomplete="off" class="form-horizontal">
          @csrf
          @method('put')
          <div class="card ">
            <div class="card-header card-header-{{$showinfo['color']}}">
              <h4 class="card-title">{{ __('Edit Statement') }} : {{ __($showinfo['type'])}}</h4>
              <p class="card-category">Del día {{$showinfo['created_at']}}</p>
            </div>
            <div class="card-body ">
              <div class="row">
                <div class="col-md-12 text-right">
                  <a href="{{ route('statements.index','statements_send') }}" class="btn btn-sm btn-{{$showinfo['color']}}">{{ __('Back to list') }}</a>
                </div>
              </div>
              <div class="row">
