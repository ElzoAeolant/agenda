@extends('layouts.app', ['activePage' => 'attendance.scan' , 'titlePage' => __('Statement Management')])
@push('head_scripts')
<style>
  video {
  width: 100%    !important;
          ...
          <div class="card-body ">
            ...
            <div class="row">
              ...
              <div class="row" id="cameras">
                <label class="col-sm-2 col-form-label">{{ __('Seleccionar cámara:') }}</label>
              </div>
            <div class="row">
...
<script>
  $(document).ready(function() {
	...
	console.log(id);
      scanner.start(camerasList[id]).catch(function(e){console.error('failed to start');});
    }
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if (response.success) {
            ...
            studentList.prepend("<tr>" + "<td>" + response.for + "</td>" + "<td class='text-"+response.color+"'>" +response.msg+response.time+ "</td>" + "</tr>");
          } else {
            ...
            let studentList = $("#studentList");
            studentList.prepend("<tr>" + "<td class='text-danger'>" + "Error" + "</td>" + "<td class='text-danger'>" + msgError + "</td>" + "</tr>");
          }
        },
    ...
    Instascan.Camera.getCameras().then(function(cameras) {
      camerasList = cameras;
      if (cameras.length > 1) {
        //scanner.start(camerasList[1]);
	/*listar las cámaras en el apartado correspondientes.*/
        let i = 0;
        camerasList.forEach(camera => {
          let divCamera = '<div class="col-sm-1">'+
                  '<div class="form-check mr-auto ml-3 mt-3">'+
                    '<label class="form-check-label">'+
                    '<input class="form-check-input" type="radio" onclick="selectcamera('+i+')" name="camerasSelected" value="'+camera.id+'" '+(i==1?'checked':'')+'>'+camera.name+
                      '<span class="form-check-sign">'+
                        '<span class="check"></span>'+
                      '</span>'+
                    '</label>'+
          ...
          cameralist.append(divCamera);
          ++i;
        });
        scanner.start(camerasList[1]).catch(function(e){console.log(e);});          
        ...
        camerasList.forEach(camera => {
          let divCamera = '<div class="col-sm-1">'+
                  '<div class="form-check mr-auto ml-3 mt-3">'+
                    '<label class="form-check-label">'+
                    '<input class="form-check-input" type="radio" onclick="selectcamera('+i+')" name="camerasSelected" value="'+camera.id+'" '+(i==0?'checked':'')+'>'+camera.name+
                      '<span class="form-check-sign">'+
                        '<span class="check"></span>'+
                      '</span>'+
                    '</label>'+
